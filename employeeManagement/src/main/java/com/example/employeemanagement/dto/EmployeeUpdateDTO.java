...
public class EmployeeUpdateDTO implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (!employeeUpdateDTO.getName().trim().matches("[a-zA-ZÀ-ỹ\\s]*")) {
            errors.rejectValue("name", "", "Tên không được có số và ký tự đặc biệt");
        }

        if (!employeeUpdateDTO.getBirthDate().equals("")) {
            try {
                LocalDate birthDate = LocalDate.parse(employeeUpdateDTO.getBirthDate());
...
//        }
        ...
        if (!employeeUpdateDTO.getSalary().trim().equals("")) {
            try {
                double salary = Double.parseDouble(employeeUpdateDTO.getSalary());
                if (salary <= 150000) {
